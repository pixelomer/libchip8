...
chip8_t *chip8_init(void) {
	...
	self->memory[0x48] = 0x12; // Jump to program (program counter is even)
	self->memory[0x45] = 0x12; // Jump to program (program counter is odd)
	return self;
}
...
void chip8_cycle(chip8_t *self) {
	...
	switch (NIBBLE(opcode, 1)) {
		...
		case 0xC:
			...
			self->registers[NIBBLE(opcode, 2)] = (uint8_t)arc4random_uniform(0x100) & BYTE(opcode, 2);
			break;
		case 0xD: {
			// DXYN (Disp)
